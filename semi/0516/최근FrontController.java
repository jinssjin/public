...
public class FrontController extends HttpServlet {
	...
	public static void ConfirmPayment(HttpServletRequest request){
	    String targetUserId = request.getParameter("user_id");
	    int postNum = Integer.parseInt(request.getParameter("post_num"));

	    PaymentDAO dao = PaymentDAO.getInstance();
	    int result = dao.ConfirmPaymentByUserAndPost(targetUserId, postNum);

	    if (result > 0) {
	        System.out.println("결제 승인 성공");
	        request.setAttribute("message", "결제 승인에 성공했습니다.");
	    } else {
	        System.out.println("결제 승인 실패");
	        request.setAttribute("message", "결제 승인에 실패했습니다.");
	    }
	}
