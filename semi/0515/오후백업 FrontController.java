...
public class FrontController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("/login.do")) {
		    ...
		    PaymentDTO existingReservation = dao.getReservationById(userId, post_num);

		    request.setAttribute("post_num", post_num); // 필요할 경우

		    if (existingReservation != null) {
		        request.setAttribute("reservationWaitInfo", existingReservation);
		        RequestDispatcher rd = request.getRequestDispatcher("/payment/reservationWaitView.jsp");
		        rd.forward(request, response);
		    } else {
		        requestReservationView(request, response); // 새 예약을 위한 정보 세팅
	...
	public void myProjectList(HttpServletRequest request) {
	    HobbyDAO dao = HobbyDAO.getInstance();
	    ArrayList<HobbyDTO> myList = new ArrayList<HobbyDTO>();
	    String items = null;
		String text = null;
	    ...
	    // 검색 조건 제거됨
	    int total_record = dao.getListCount(items, text);
	    myList = dao.getMyBoardList(pageNum, limit,id); // 수정 필요: 검색 조건 없이 페이징된 리스트 가져오기
	...
	public void myPayMentList(HttpServletRequest request) {
	    ...
	    // 검색 조건 제거됨
	    int total_record = dao.getListCount(items, text);
	    myPayList = dao.getMyPaymentList(pageNum, limit,id); // 수정 필요: 검색 조건 없이 페이징된 리스트 가져오기
	...
	public void requestBoardList(HttpServletRequest request) {
	    ...
	    if (userClass == null) userClass = "U"; // 기본값 사용자

	    // 등급에 따라 전체 게시글 또는 삭제 제외된 게시글만 조회
	...
	public void requestBoardView(HttpServletRequest request) {
		...
		request.setAttribute("pageNum", pageNum);
		request.setAttribute("board", board);
		request.setAttribute("comments", commentList); // 👉 댓글 리스트도 전달
	}
...
public void requestReservationWaitView(HttpServletRequest request, HttpServletResponse response) {
		...
		PaymentDTO reservationWaitInfo = dao.getReservationById(userId,post_num);

		request.setAttribute("post_num", post_num);
		request.setAttribute("reservationWaitInfo", reservationWaitInfo);
